# 61.- Un amigo tuyo ha decidido dar clases particulares de inglés y te 
# ha pedido que le ayudes a apuntar qué clases tiene pendientes.
# 
# Para cada clase quiere apuntar el nombre del alumno, la fecha (como 
# texto, por ejemplo, "2026-02-16") y la hora (como texto, por ejemplo 
# "19:05"). Además, cuando ya ha impartido la clase (pero no antes) 
# quiere poder anotar cuánto ha cobrado por ella.
# 
# El programa debe:
# 
# 1. Mostrar un menú que recuerde al usuario todas las opciones 
# disponibles y se repita hasta que se escoja la opción "F" (Fin), ya sea 
# en mayúsculas o en minúsculas. (1 punto) 
# 
# 2. Permitir añadir un nueva clase, que se guardará en formato de 
# diccionario, y que a su vez será parte de una lista llamada "clases". 
# Para cada clase, se le pedirá nombre, fecha y hora, y el importe se 
# anotará como 0 (para indicar que está pendiente) (2 puntos) 
# 
# 3. Poder ver las clases pendientes (las que tienen importe 0). Para 
# cada clase, se mostrará su número (contando desde 1), su fecha, su hora 
# y el nombre del alumno. (1 punto)

# 4. Marcar una clase como impartida. Preguntará el número de clase. Si 
# el número corresponde a una clase existente y cuyo importe es cero, se 
# pedirá el importe y se anotará. Si el número es incorrecto o se trata 
# de una clase que ya tenía importe, se avisara al usuario.

# 5. Calcular el importe total de las clases. Mostrará la cantidad de 
# clases, el importe total que se ha obtenido y el importe medio por cada 
# clase.

# 6. Modificar los detalles de una clase, a partir de su número. Si el 
# número corresponde a una clase existente, se volverá a pedir su fecha, 
# hora, nombre e importe. Si el número es incorrecto, se avisara al 
# usuario. (1 punto)

# 7. Los datos se guardarán tras cada cambio (cuando se añada una clase, 
# se marque como impartida, o cuando se modifique). (1 punto)

# 8. Los datos anteriores, si existían, se cargarán al comenzar la 
# ejecución del programa. (1 punto)

# 9. Es deseable que el programa esté dividido en funciones para 
# facilitar su legibilidad y su mantenibilidad. Estas funciones no 
# compartirán variables globales sino que recibirán la lista de libros 
# como parámetro, y la devolverán modificada en los casos en los que sea 
# necesario. Los nombres de las variables deben ser claros y el programa 
# debe resultar fácil de seguir. (1 punto)

# Versión 6b: menú + añadir + ver + marcar impartida + total
#   + guardar + excepciones

def cargar():
    clases =  [ ]
    try:
        fichero = open("clases.txt", "r")
        linea = fichero.readline().rstrip()
        while linea:
            #print(linea)

            trozos = linea.split(";")
            clase =  {
                    "alumno": trozos[0],
                    "fecha": trozos[1],
                    "hora": trozos[2],
                    "importe": float(trozos[3])
            }
            clases.append( clase )

            linea = fichero.readline().rstrip()
        fichero.close()
    except:
        print("No había datos")
    return clases


def guardar(clases):
    fichero = open("clases.txt","w")
    for clase in clases:
        fichero.write(clase["alumno"]+";"+clase["fecha"]+";"+
            clase["hora"]+";"+str(clase["importe"])+"\n")
    fichero.close()
   

def anyadir(clases):
    nombre = input("Dime el nombre del alumno: ")
    fecha = input("Dime la fecha de la clase: ")
    hora = input("Dime la hora de la clase: ")
    
    clase =  {
        "alumno": nombre,
        "fecha": fecha,
        "hora": hora,
        "importe": 0
    }
    clases.append( clase )
    return clases


def ver_pendientes(clases):
    for i in range(len(clases)):
        clase = clases[i]
        if clase["importe"] == 0:
            print(i+1,
                clase["fecha"], 
                clase["hora"], 
                clase["alumno"])


def ver_total(clases):
    total = 0
    for clase in clases:
        total += clase["importe"]
    print("Cantidad de clases:", len(clases))
    print("Total cobrado:", total)
    print("Media:", total / len(clases))


def marcar_impartida(clases):
    numero = int(input("¿Qué número de clase has impartido? ")) - 1
    if numero >= 0 and numero < len(clases):
        if clases[numero]["importe"] == 0:
            dinero = int(input("Cuánto has cobrado? " ))
            clases[numero]["importe"] = dinero
        else:
            print("Esa clase ya se ha cobrado")
    else:
        print("Número de clase no válido")
    return clases

def modificar(clases):
    numero = int(input("¿Qué número de clase modificar? ")) - 1
    if numero >= 0 and numero < len(clases):
        nombre = input("Dime el nombre del alumno: ")
        fecha = input("Dime la fecha de la clase: ")
        hora = input("Dime la hora de la clase: ")
        importe = float(input("Dime el importe de la clase: "))
        
        clase =  {
            "alumno": nombre,
            "fecha": fecha,
            "hora": hora,
            "importe": importe
        }
        clases[numero] = clase
    else:
        print("Número de clase no válido")
    return clases


terminado = False
clases = cargar()

while not terminado:
    print("1- Añadir una clase")
    print("2- Ver clases pendientes")
    print("3- Marcar clase como impartida")
    print("4- Calcular importe de las clases")
    print("5- Modificar una clase")
    print("F- Fin")
    
    opcion = input("¿Qué opción? ").upper()
    
    if opcion == "F":
        terminado = True

    elif opcion == "1":
        clases = anyadir(clases)
    
    elif opcion == "2":
        ver_pendientes(clases)
    
    elif opcion == "3":
        clases = marcar_impartida(clases)
    
    elif opcion == "4":
        ver_total(clases)
    
    elif opcion == "5":
        clases = modificar(clases)

guardar(clases)

